...
class Mustache {
  ...
  abstract class SimpleMap(m:Map[String, Any]) extends util.Map[String, AnyRef]{
    ...
    override def values(): util.Collection[Any] = {
      val arrayList = new util.ArrayList[Any](m.size)
      m.foreach(e => arrayList.add(recursiveToJava(e._2)))
      arrayList
    }
    // NOT IMPLEMENTED
    override def keySet(): util.Set[String] = ???
    override def entrySet(): util.Set[util.Map.Entry[String, Any]] = ???
    ...
    override def put(key: String, value: Any): AnyRef = ???
